...
# Сохраняем список игроков в session_state, чтобы выбранные данные запоминались
...
def leaderboard_page():
    ...
    # Кнопка Обновить (перерисовка страницы)
    if st.button("Обновить"):
        st.experimental_rerun()

    # Контейнер с ограниченной высотой и уменьшённым шрифтом, чтобы таблица помещалась без прокрутки
    st.markdown("""
    <style>
    ...
    # Сортировка игроков по убыванию баллов
...
def team_assignment_page():
    ...
    if st.button("Перемешать команды"):
        ...
        best_diff = float('inf')
        best_partition = None
        # Перебираем все комбинации по 4 игрока
